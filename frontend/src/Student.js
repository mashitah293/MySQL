...
function Student() {
    ...
    useEffect(() => {
        axios.get("http://localhost:5000/") // Send GET request to the server to fetch all students
                ...
                <table className='table'>
                    <thead>
                        <tr>
                            ...
                            <th>marks</th> {/* Table header for student email */}
                            <th>grade</th> {/* Table header for student email */}
                            <th>city</th> {/* Table header for student email */}
                    ...
                    <tbody>
                        {
                            student.map((data, i) => ( // Map through the student array to display each student's details
                                <tr key={i}>
                                    ...
                                    <td>{data.marks}</td> {/* Display student email */}
                                    <td>{data.grade}</td> {/* Display student email */}
                                    <td>{data.city}</td> {/* Display student email */}
                                    <td>
